...
- `src/Agent/Support/BootableHealthCheck.php`, `src/Agent/PipelineStages.php`, `src/Agent/Roles/Support/ContextGuards.php`, `src/Llm/Reasoner.php`, `src/Agent/Roles/Support/ReleaseActionsRunner.php`, `src/Agent/Roles/Support/FileChangeApplier.php`, and `src/Agent/Roles/Support/FindingsEvaluator.php`: named, single-responsibility pieces extracted out of `src/Agent/` (see Changed).
...
- Reorganized `src/Agent/` into smaller, named, single-responsibility pieces: no behavior change and no public constructor signature change. `AbstractRoleAgent`, `AgentOrchestrator`, and `CallbackAgent` now share their `boot()`/`isHealthy()`/`health()` boilerplate via the `BootableHealthCheck` trait instead of each hand-implementing it. `AbstractRoleAgent` composes `ContextGuards` (`requireField`/`requireHistory`) and a new `SquirrelForge\Llm\Reasoner` (prompt building/JSON parsing for `reason()`) instead of implementing them inline. `ReleaseAgent` delegates its real, side-effecting release actions to `ReleaseActionsRunner`, keeping only the pure gate-check itself. `DeveloperAgent` delegates per-file-change application to `FileChangeApplier`. `SecurityAgent` and `PerformanceAgent` both delegate their identical critical/warning/approved severity rule to `FindingsEvaluator`. `ArchitectAgent`, `PlannerAgent`, `ReviewerAgent`, `DocumentationAgent`, `AgentRegistry`, `AgentServiceProvider`, and `AgentPipelineModule` are unchanged.
