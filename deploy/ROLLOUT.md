...
Release tags continue from publication into the `protected-rollout` job. The job targets the GitHub `production` environment; configure that environment with required reviewers, deployment-branch or tag protections, and the minimum Kubernetes credential secret. Workflow concurrency permits only one production release and never cancels an active rollout. This job promotes only the main Engine API image (`needs: publish`).

The credential-provider image is built, scanned, and published by its own independent `credential-provider-gate`/`publish-credential-provider` jobs, and now has its own staged-rollout counterpart, `protected-rollout-credential-provider` (`needs: publish-credential-provider`) — the identical admission/canary/stable/observation/rollback sequence via the same `bin/rollout-release.sh`/`bin/observe-release.sh`/`bin/rollback-release.sh` scripts, targeting the same `production` GitHub environment but its own concurrency group (`squirrelforge-credential-provider-production-release`, so the two releases never block each other) and its own Kubernetes namespace/Deployments/vars/secrets, all prefixed `SQUIRRELFORGE_CREDENTIAL_PROVIDER_ROLLOUT_*`/`SQUIRRELFORGE_CREDENTIAL_PROVIDER_OBSERVE_*` so they never collide with the main image's own. Start with `deploy/credential-provider-rollout-policy.env.example` and `deploy/kubernetes/credential-provider-canary-deployment.yaml` — see `deploy/CREDENTIAL-PROVIDER-CONTRACT.md` for the full picture. Both jobs reuse the same self-hosted `squirrelforge-production` runner and the same `SQUIRRELFORGE_KUBECONFIG_BASE64` cluster credential; scope that credential's Kubernetes RBAC to both services' namespaces/Deployments, never cluster-admin.

Neither the GitHub `production` environment nor either image's Kubernetes namespace/Secret/stable-Deployment exists yet as of this writing. `deploy/PRODUCTION-STANDUP.md` is the one-time operator runbook for creating both — do that before either `protected-rollout` job can succeed.
