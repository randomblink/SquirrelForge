...
`.github/workflows/flock-deployment-gate.yml` now builds, smoke-tests, SBOMs, vulnerability-scans, and — on a `v*` tag — publishes and Cosign-signs this image with build-provenance and CycloneDX SBOM attestations, via its own `credential-provider-gate`/`publish-credential-provider` jobs mirroring the main Engine API image's `deployment-gate`/`publish` (see `deploy/PRODUCTION.md`). It also has its own staged-rollout pipeline, `protected-rollout-credential-provider`, mirroring `protected-rollout` with the same admission → canary → stable → observation → verified-rollback sequence via the identical `bin/rollout-release.sh` / `bin/observe-release.sh` / `bin/rollback-release.sh` scripts — an independent promotion of this service's own canary/stable Kubernetes Deployments (`deploy/kubernetes/credential-provider-canary-deployment.yaml`, `deploy/credential-provider-rollout-policy.env.example`), never a second tag or replica of the main image's rollout. Standing up the real Kubernetes namespace, Secret, and metrics endpoints this pipeline reads from remains an operator responsibility — the workflow wiring and scripts are real, but nothing deploys until those resources exist.
