...
`publish-credential-provider` applies the identical SBOM, vulnerability-gate, evidence, checksum-verification, GHCR push, Cosign signing, and provenance/SBOM attestation treatment to the credential-provider image independently, publishing it as its own package (`ghcr.io/<owner>/<repo>/credential-provider:<tag>`) rather than a second tag on the main image. It continues into its own `protected-rollout-credential-provider` job — an independent admission → canary → stable → observation → verified-rollback promotion of the credential-provider service's own Kubernetes Deployments, reusing the exact same `bin/rollout-release.sh`/`bin/observe-release.sh`/`bin/rollback-release.sh` scripts as the main image's `protected-rollout` job but pointed at distinctly-named vars/secrets (`SQUIRRELFORGE_CREDENTIAL_PROVIDER_ROLLOUT_*`/`SQUIRRELFORGE_CREDENTIAL_PROVIDER_OBSERVE_*`) so the two releases never share a namespace, deployment, or receipt file. See `deploy/kubernetes/credential-provider-canary-deployment.yaml` and `deploy/credential-provider-rollout-policy.env.example`.
