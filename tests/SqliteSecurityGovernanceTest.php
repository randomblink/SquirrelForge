...
final class SqliteSecurityGovernanceTest extends TestCase
{
    ...
    {
        $path = sys_get_temp_dir() . "/squirrelforge-security-governance-{$label}-" . bin2hex(random_bytes(8)) . '.sqlite';
    ...
    /**
     * @return array{request_id: string, policy_context: array<string, mixed>}
     */
    ...
    public function testApprovesWhenAnAllowingPolicyApplies(): void
    {
        ...
        $policyEngine->registerPolicy('p1', ['category' => 'authorization', 'priority' => 1, 'condition' => $this->boolCondition('ready'), 'effect' => 'allow']);
    ...
    {
        $policyEngine = new SqlitePolicyEngine($this->tempPath('policy'), new RuleEngine());
        $policyEngine->registerPolicy('p1', ['category' => 'authorization', 'priority' => 1, 'condition' => $this->boolCondition('ready'), 'effect' => 'allow_with_conditions']);
        $governance = new SqliteSecurityGovernance($this->tempPath('gov'), $policyEngine);
    ...
    {
        $policyEngine = new SqlitePolicyEngine($this->tempPath('policy'), new RuleEngine());
        $policyEngine->registerPolicy('p1', ['category' => 'authorization', 'priority' => 1, 'condition' => $this->boolCondition('ready'), 'effect' => 'require_review']);
        $governance = new SqliteSecurityGovernance($this->tempPath('gov'), $policyEngine);
    ...
    {
        $policyEngine = new SqlitePolicyEngine($this->tempPath('policy'), new RuleEngine());
        $policyEngine->registerPolicy('p1', ['category' => 'authorization', 'priority' => 1, 'condition' => ['type' => 'boolean', 'field' => 'missing_field', 'equals' => true], 'effect' => 'allow']);
        $governance = new SqliteSecurityGovernance($this->tempPath('gov'), $policyEngine);
    ...
    {
        $policyEngine = new SqlitePolicyEngine($this->tempPath('policy'), new RuleEngine());
        $policyEngine->registerPolicy('p1', ['category' => 'authorization', 'priority' => 1, 'condition' => $this->boolCondition('ready'), 'effect' => 'prohibit']);
        $governance = new SqliteSecurityGovernance($this->tempPath('gov'), $policyEngine);
    ...
    public function testRiskAssessmentAndComplianceStatusDefaultWhenNotSupplied(): void
    {
        $policyEngine = new SqlitePolicyEngine($this->tempPath('policy'), new RuleEngine());
        $policyEngine->registerPolicy('p1', ['category' => 'authorization', 'priority' => 1, 'condition' => $this->boolCondition('ready'), 'effect' => 'allow']);
        $governance = new SqliteSecurityGovernance($this->tempPath('gov'), $policyEngine);
        ...
        $this->assertSame('not_supplied', $result['risk_assessment']);
        $this->assertSame('not_evaluated', $result['compliance_status']);
    }

    public function testEveryReviewIsRecordedWithFinalOutcomeDerivedFromDecision(): void
    {
        $policyEngine = new SqlitePolicyEngine($this->tempPath('policy'), new RuleEngine());
        $policyEngine->registerPolicy('p1', ['category' => 'authorization', 'priority' => 1, 'condition' => $this->boolCondition('ready'), 'effect' => 'deny']);
        $governance = new SqliteSecurityGovernance($this->tempPath('gov'), $policyEngine);

        $result = $governance->review($this->request(['risk_assessment' => 'high', 'compliance_status' => 'compliant', 'reviewer_component' => 'incident_manager']));
        $record = $governance->get($result['governance_id']);

        $this->assertSame('Rejected', $record['decision_type']);
        $this->assertSame('not_approved', $record['final_outcome']);
        $this->assertSame('high', $record['risk_assessment']);
        $this->assertSame('compliant', $record['compliance_status']);
        $this->assertSame('incident_manager', $record['reviewer_component']);
    }
}
