...
Observed across `SF-REVIEW-094` (Plugin category consistency review) and `SF-REVIEW-103` (Performance category consistency review): three separate entries — `WP-ERROR-017`, `WP-ERROR-031` (both `SF-REVIEW-094`), and `WP-ERROR-035` (`SF-REVIEW-103`) — each carried a non-standard Section 16 intro sentence, deviating from the wording 29 of this repository's 31 entries use ("The following are cited as they exist in this repository, or as conceptual distinctions where noted."). In every one of the three instances, neither the entry's own author review nor its own independent review caught the deviation; it surfaced only when a later category-level consistency review compared the wording across every entry in the category side by side.

This is a distinct failure mode from the ownership-conflict and cross-document-completeness patterns already tracked in this file: it is a pure style-conformance drift with no technical or boundary content at stake, invisible to a single-entry review because there is nothing to compare against until multiple entries in the same category sit next to each other. Three occurrences across two different categories suggest this is a catalog-wide conformance question, not a category-specific one — worth considering, when time permits, whether `scripts/validate-repo.sh` should gain a fourth check comparing each entry's own Section 16 intro sentence against the majority wording directly, catching this at the moment an outlier is introduced rather than waiting for the next category consistency review to notice it.

Not acted on now; each of the three instances was corrected directly within its own category consistency review. Disclosed here as a repository-wide pattern now crossing the threshold this project has used elsewhere (`SF-REVIEW-075`/`078`/`087`) to justify recording a defect class, though not yet enough evidence on its own to justify the validator change without further consideration of whether "majority wording" is a stable target as the catalog continues to grow.

---

## 2026-07-14 — WP-ERROR-013's own Common Causes list accumulates cross-references as new bootstrap-adjacent mechanisms are documented elsewhere

Observed across `SF-REVIEW-093` (`WP-ERROR-032`'s own independent review) and `SF-REVIEW-103` (Performance category consistency review): `WP-ERROR-013`'s own Common Causes list has been extended twice — once to name a plugin-update interruption as a cause (citing `WP-ERROR-032`), and once to name cross-file OPcache bytecode inconsistency as a cause (citing `WP-ERROR-035`) — each time because a newly-authored entry elsewhere in the catalog documented a condition capable of producing the exact site-wide PHP fatal error `WP-ERROR-013` already owns generically, without `WP-ERROR-013`'s own text yet naming that specific cause. (A third apparent instance was considered and ruled out on inspection: `WP-ERROR-017`'s own must-use-plugin cause was already present in `WP-ERROR-013`'s original authoring, not added as a later correction, and the `WP-ERROR-031`-related correction that same review cycle produced was applied to `WP-ERROR-017`'s own text, not `WP-ERROR-013`'s — the confirmed count is two, not three.)

Both instances follow the same shape: `WP-ERROR-013`'s own Section 17 disclaimer already anticipates that "cause-specific conditions... may each be documented by a separate, independently created `WP-ERROR` entry without altering this one," which correctly permits a new entry to claim a specific cause without requiring `WP-ERROR-013` itself to change — but does not, on its own, keep `WP-ERROR-013`'s own Common Causes list current as a *reader's* first stop for diagnosing a bootstrap fatal error. This is a different class of gap from the two staleness patterns already tracked in this file: it is not a citation going stale or a taxonomy missing an existing entry's claim, but a foundational, general-condition entry naturally falling behind a growing set of more specific entries that can each produce its own symptom.

Not acted on now; two confirmed data points is the same evidentiary bar this project has previously required before treating a pattern as established (`SF-REVIEW-078`'s own stale-hedge check), and both instances were caught only during a *different* entry's own independent review or the category's own consistency review, never proactively. Worth considering, when time permits, whether authoring a new entry whose Severity section describes a fatal-error-capable manifestation should include, as a standard step, checking whether `WP-ERROR-013`'s own Common Causes list needs a corresponding addition — the same discipline already applied ad hoc in both instances so far, made explicit rather than rediscovered each time.
