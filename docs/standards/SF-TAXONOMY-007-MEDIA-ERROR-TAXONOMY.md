...
- `WP-ERROR-020` explicitly excludes upload-size rejection from its own capacity scope. The enforcing paths are PHP's `upload_max_filesize`/`post_max_size` request parsing and, on multisite only, WordPress's `check_upload_size()`; `wp_max_upload_size()` and `upload_size_limit` only calculate advisory UI text. These paths operate independently of free capacity on the WordPress destination volume and are owned here by `WP-ERROR-036`. Independent verification during this taxonomy's original drafting found that no earlier entry, including `WP-ERROR-014`/`015` (PHP Runtime), owned the condition. See Section 3.
...
| 1.3 | 2026-07-14 | WP-ERROR-038 reached Production Ready (SF-REVIEW-110 author review, no findings; SF-REVIEW-111 independent review, which corrected a precision qualifier for an illustrative example within the entry itself, rather than finding a defect in this taxonomy). Status column updated from Planned to Existing, Production Ready. No further boundary content changed; this entry required no revision to this taxonomy. All three planned Media entries are now Existing, Production Ready. | Frozen |
| 1.4 | 2026-07-15 | Post-certification change per **SF-SPEC-013** Section 5.6, prompted by `WP-VERIFICATION-001` (the first Reference Implementation runtime-verification record in this catalog): Section 3's own `WP-ERROR-037` row named a filter, `allow_unfiltered_uploads` (lowercase), that a full-text search of WordPress core 7.0.1 confirmed does not exist anywhere in that codebase — corrected to name the real mechanism, the `ALLOW_UNFILTERED_UPLOADS` `wp-config.php` constant. No entry ownership, boundary, or planned-entries content changed; this was a factual-accuracy correction to this taxonomy's own descriptive text, not a scope revision. Companion correction to `WP-ERROR-037` itself (Version 1.0 → 1.1) reviewed via `SF-REVIEW-149`/`150`; category re-certified via `SF-REVIEW-151` (consistency) and `SF-REVIEW-152` (baseline). | Frozen |
| 1.5 | 2026-07-15 | Post-certification change per **SF-SPEC-013** Section 5.6, prompted by `WP-VERIFICATION-003`: Section 3's own `WP-ERROR-036` row described WordPress's own `wp_max_upload_size()` filter as a generic further-restriction mechanism; direct runtime testing and source verification found this function is used only to render UI display text and performs no enforcement, on any installation type — the real, further-restriction mechanism beyond the two PHP directives is `check_upload_size()`, registered only on a multisite installation. Corrected the row accordingly. No entry ownership or boundary content changed. Companion correction to `WP-ERROR-036` itself (Version 1.0 → 1.1) reviewed via `SF-REVIEW-155`/`156`; category re-certified via `SF-REVIEW-157` (consistency) and `SF-REVIEW-158` (baseline), producing Media Knowledge Baseline v3. | Frozen |
| 1.6 | 2026-07-16 | Post-certification fidelity correction prompted during `WP-VERIFICATION-005`: Section 2 still quoted WP-ERROR-020's obsolete pre-`WP-VERIFICATION-003` mechanism description even though this taxonomy's own Version 1.5 row was correct. Updated only that live cross-category explanation. Media ownership and planned entries are unchanged. Re-certified via `SF-REVIEW-173`/`174`, producing Media Knowledge Baseline v4. | Frozen |
