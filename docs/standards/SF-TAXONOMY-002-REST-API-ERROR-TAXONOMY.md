...
**REST API** owns failures in WordPress's built-in REST API (`wp-json`) request-handling pipeline, following that pipeline's own natural lifecycle: route resolution, request acceptance (authentication, authorization, and argument validation), and callback execution/response generation.
...
- Generic `wp-admin` cookie or session authentication unrelated to a REST request specifically — Authentication category (`WP-ERROR-024` for login/session establishment, `WP-ERROR-025` for cookie/session persistence, `WP-ERROR-026` for non-REST capability/authorization denial; see `SF-TAXONOMY-003`).
...
- Web-server routing or rewrite/permalink failure that prevents a pretty `/wp-json/...` request from populating `rest_route` and reaching WordPress's REST dispatcher. Success through the rewrite-independent `?rest_route=...` form proves the route exists and makes the pretty-URL failure a diagnostic distinction, not `WP-ERROR-021`'s `rest_no_route` condition. REST API owns only failures after its request-handling pipeline is reached.
...
|---|---|---|---|
| `WP-ERROR-021` | WordPress REST API Route Not Found | Route resolution after REST dispatch is reached — no path-and-method handler matches because the route was never registered, the namespace/path or method is incorrect, or the registered route was removed from the match table; WordPress returns `rest_no_route` with HTTP 404 | Existing, Production Ready |
| `WP-ERROR-022` | WordPress REST API Access Denied | Request acceptance — a callback *has* been identified for the route, but the request is rejected before that callback's own business logic runs, because identity verification (cookie/nonce, Application Passwords, or another authentication mechanism) fails, the route's own `permission_callback` denies the request, or the request's own arguments fail WordPress's built-in schema/type validation — regardless of which specific mechanism or gate produced the rejection | Existing, Production Ready |
| `WP-ERROR-023` | WordPress REST API Response Error | Callback execution and response generation — the request has been accepted (route resolved, authenticated, authorized, and its arguments validated) and the callback's own business logic has begun running; every failure from that point forward (an exception or fatal error during execution, an invalid or non-serializable return value, or a `WP_Error` generated by the callback's own logic) belongs here, regardless of that failure's own underlying root cause | Existing, Production Ready |

All three entries now exist and are Production Ready; the REST API category's planned baseline is complete. Nothing else is currently planned for this category. Any future addition to this table is a revision to this document, not an ad hoc decision made while authoring an unrelated entry, per **SF-SPEC-013** Section 5.7.
...
Incoming REST request
  ...
  (can the request reach a callback at all?)
        │
        ▼
  022 — Request acceptance
  (authentication, permission_callback, argument/schema validation —
   is this request allowed to proceed to business logic?)
        │
        ▼
  023 — Callback execution / response generation
  (did the accepted request's own business logic execute and respond correctly?)
```
...
**Where argument/schema validation belongs:** WordPress's own built-in request-argument validation (`validate_callback`/`sanitize_callback`, invoked via `WP_REST_Request::has_valid_params()`) runs during `WP_REST_Server::dispatch()`, mechanically before the route's own callback function is invoked in most cases. This taxonomy deliberately does not resolve that placement by implementation detail. It resolves it by observable engineering state, consistent with how every other category in this repository draws its boundaries: the callback's own business logic has not yet begun, and the request has been rejected as unacceptable rather than having failed while executing. Argument/schema validation therefore belongs with `WP-ERROR-022` (request acceptance), not `WP-ERROR-023` — the final gate before business logic runs, alongside authentication and the permission callback, not the first step of execution. This decision is recorded here so it does not need to be inferred, or independently re-decided, when `WP-ERROR-022` and `WP-ERROR-023` are drafted.
...
| 1.2 | 2026-07-14 | Resolved the argument/schema-validation placement question `SF-REVIEW-045` flagged as Informational: assigned to `WP-ERROR-022` (request acceptance) rather than `WP-ERROR-023` (callback execution), on the basis of observable engineering state — the callback's own business logic has not yet begun — consistent with this repository's established boundary philosophy, rather than WordPress's own internal invocation order. Section 3's Owns column, Section 4's diagram, and Section 2's top-line description updated accordingly. No entry added, removed, or renumbered. | Frozen |
| 1.3 | 2026-07-14 | Corrected per the REST API category consistency review: WP-ERROR-021, 022, and 023 all reached Production Ready, but this table's Status column still read `Planned` for all three. Updated all three to `Existing, Production Ready`, completing the three-entry REST API baseline. No boundary content changed. | Frozen |
| 1.4 | 2026-07-14 | Corrected per `SF-REVIEW-075` (`WP-ERROR-026` independent review, IF-1): Section 2's "Authentication category (once a taxonomy exists for it)" hedge was stale now that `SF-TAXONOMY-003` exists and has real entries. Updated to cite `WP-ERROR-024`/`025`/`026` specifically. No boundary content changed. | Frozen |
| 1.5 | 2026-07-16 | Post-certification correction prompted by the source gate preceding `WP-VERIFICATION-007`: removed rewrite/permalink failures that prevent REST dispatch from WP-ERROR-021 ownership and retained them as a required diagnostic distinction. WP-ERROR-021 now owns only WordPress-generated `rest_no_route` after path-and-method matching. Entry count and the three-stage REST pipeline are unchanged. Reviewed and re-certified via `SF-REVIEW-183`–`186`. | Frozen |
