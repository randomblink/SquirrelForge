...
|---|---|---|---|
| 1.0 | 2026-07-13 | Initial specification, established as part of the framework's initial authoring pass (commit `7f29178`), then reviewed and corrected by `SF-REVIEW-002` (Specification Library Review) as part of a unified pass across `SF-SPEC-001` through `011` (Phases 1–6, 2026-07-12): Entry 2 (Phase 2) added a citation at then-§2.2 to **SF-SPEC-002** for the excluded "runtime evidence collection" subject; the Ownership Conflict Validation gate within that same entry additionally found and resolved a duplicate "Evidence Categories" list this file shared with **SF-SPEC-002**, rewriting this file's own list to cite **SF-SPEC-002** as authoritative and retaining only the two categories unique to this specification's broader scope. `SF-REVIEW-002` predates, and does not use, the Class A/Class B/Class C reviewer-classification system **SF-SPEC-012** later introduced. | Draft — reviewed via `SF-REVIEW-002` (predates Class A/B system) |
