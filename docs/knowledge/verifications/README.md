...
| `WP-VERIFICATION-017` | `WP-ERROR-009` | Server statement-timeout and lock-wait boundaries verified on usable connections; PHP, stock-driver, and gateway paths explicitly deferred | `WP-ERROR-009` v1.0→1.2 through two separate correction cycles; Database Knowledge Baseline v3 |
| `WP-VERIFICATION-018` | `WP-ERROR-018` | Default initial database-error presentation and optional `db-error.php` override verified; cause-specific ownership preserved | `WP-ERROR-018` v1.0→1.1; Database Knowledge Baseline v4 |
