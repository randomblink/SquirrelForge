...
A login attempt — submitted via `wp-login.php`, XML-RPC, or a programmatic `wp_signon()` call — is evaluated by WordPress's own core credential-verification pipeline (`wp_authenticate()` and its `authenticate` filter chain) and rejected because the supplied identity and credential do not correspond to a valid, matching user account. For XML-RPC, that pipeline can also evaluate an Application Password through Core's default `wp_authenticate_application_password()` handler. This entry ends the moment WordPress decides the user is not authenticated; it does not extend into anything that happens before that decision (a request blocked before reaching the authentication code) or after it (cookie issuance and persistence, capability checks, or request-origin verification).

---

# 4. Primary Failure Mode

A login attempt reaches WordPress's own `wp_authenticate()` pipeline. Its default core handlers include `wp_authenticate_username_password`, `wp_authenticate_email_password`, `wp_authenticate_application_password` (for REST or XML-RPC API requests), and `wp_authenticate_spam_check` (Multisite only). The applicable handler determines that no valid account matches the submitted username or email, that the submitted password does not match the account's stored hash, or, for an XML-RPC Application Password, that the submitted Application Password is unavailable or invalid. `wp_authenticate()` returns a `WP_Error` (or a plugin-added `authenticate` filter callback returns one first in the chain). Core fires `wp_login_failed` for rejected results other than `empty_username` and `empty_password`; those two empty-field errors are returned without that action. No authenticated session is established as a result.
...
- **`WP-ERROR-022` — WordPress REST API Access Denied**: covers authentication and authorization failures *within a REST API request specifically*, including REST Application Password and `X-WP-Nonce` handling. A REST request that fails because of invalid Application Password credentials belongs to `WP-ERROR-022`, not here. An XML-RPC request using an Application Password instead remains within this entry because `wp_xmlrpc_server::login()` invokes the `wp_authenticate()` pipeline; this does not make an Application Password failure a generic `wp-login.php` form failure.
...
- The default `authenticate` filter callbacks: `wp_authenticate_username_password` and `wp_authenticate_email_password` (both core, priority 20), which perform username/email lookup and `wp_check_password()` comparison; `wp_authenticate_application_password` (core, priority 20), which participates only for REST or XML-RPC API requests; and `wp_authenticate_spam_check` (Multisite only, priority 99), which rejects an authenticated account flagged as spam.
...
- The `wp_login_failed` action hook, fired for rejected `WP_Error` results except Core's `empty_username` and `empty_password` errors, commonly used by logging and security plugins to record failed attempts (recording is in scope; the plugin's own subsequent blocking decision is not, per Section 6).
...
- For rejected results other than `empty_username` or `empty_password`, the `wp_login_failed` action fires and any logging or security plugin observing it can record an entry. Its presence is confirmatory evidence; its absence does not rule out either empty-field error, and a log showing the request never reached this point indicates the plugin/WAF-blocking exclusion in Section 7.
...
2. Capture the exact error message and `WP_Error` code returned (`invalid_username`, `incorrect_password`, `invalid_email`, `empty_username`, `empty_password`, an Application Password code, or a plugin-specific `authenticate`-filter code). Treat a `wp_login_failed` record as corroboration only after confirming the result is not one of Core's two empty-field errors.
...
1. [WP-ERROR-025 — WordPress Authentication Cookie Invalid or Expired](WP-ERROR-025-AUTHENTICATION-COOKIE-INVALID-OR-EXPIRED.md) — exists in this repository and is Production Ready; see Section 6 (Distinction) above for how the session-layer condition differs from this entry's own identity-verification condition.
2. [WP-ERROR-026 — WordPress Capability or Role Authorization Denied](WP-ERROR-026-CAPABILITY-OR-ROLE-AUTHORIZATION-DENIED.md) — exists in this repository and is Production Ready; see Section 6 (Distinction) above for the authentication-versus-authorization distinction.
3. [WP-ERROR-027 — WordPress Nonce Verification Failure, Non-REST](WP-ERROR-027-NONCE-VERIFICATION-FAILURE-NON-REST.md) — exists in this repository and is Production Ready; see Section 6 (Distinction) above for why a nonce failure is independent of authentication state.
...
**Version 1.1 (2026-07-17):** post-certification correction through **SF-SPEC-013** Section 5.6, prompted by the source gate preceding `WP-VERIFICATION-010`. Corrected the default Core authentication-handler list to include Application Password handling; classified XML-RPC Application Password authentication within this entry's `wp_authenticate()` pipeline while retaining REST ownership in `WP-ERROR-022`; narrowed `wp_login_failed` to exclude Core's `empty_username` and `empty_password` results; and updated stale sibling statuses. Reviewed via `SF-REVIEW-210`/`211`; Authentication re-certified via `SF-REVIEW-212`/`213` as Knowledge Baseline v2.
