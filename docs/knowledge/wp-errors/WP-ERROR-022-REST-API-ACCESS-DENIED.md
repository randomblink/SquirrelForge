...
- **Authentication** — identity verification via cookie authentication and its accompanying nonce (`X-WP-Nonce`), Application Passwords, or a custom mechanism (a JWT or OAuth plugin) fails, is invalid, or is insufficient for the request to proceed. Core mechanisms do not all traverse one universal handler sequence: cookie and Application Password identity callbacks participate in `determine_current_user`; `rest_cookie_check_errors()` and `rest_application_password_check_errors()` separately surface their mechanism-specific REST result through `rest_authentication_errors`. A custom REST authentication callback on `rest_authentication_errors` should return `null` when its mechanism is not being attempted, `true` when it succeeds, or `WP_Error` when it fails, while preserving any non-null result from an earlier callback.
...
- Cookie authentication and its accompanying nonce requirement: `rest_cookie_check_errors()` (`wp-includes/rest-api.php`), the `X-WP-Nonce` HTTP header or `_wpnonce` request parameter, and `wp_verify_nonce()`. When no nonce is supplied, Core sets the current user to anonymous and returns `true`; downstream permission logic then evaluates an unauthenticated requester. When a nonce is supplied but invalid or expired, Core returns `rest_cookie_invalid_nonce` with HTTP 403.
- Application Passwords (`wp-includes/class-wp-application-passwords.php`), WordPress's own built-in credential mechanism for external REST clients, authenticated via HTTP Basic Auth using a WordPress username and a generated application-password string. Core supports the mechanism by default on SSL sites and when `wp_get_environment_type()` is `local`; `wp_is_application_passwords_available` can override the resulting global availability (and some environments disable it).
- The mechanism-specific authentication paths: cookie and Application Password identity callbacks on `determine_current_user`, followed by their separate REST error/result callbacks (`rest_cookie_check_errors()` and `rest_application_password_check_errors()`) on `rest_authentication_errors`. Custom mechanisms may participate in either or both filters according to their implementation; there is no universal cookie-then-Application-Passwords-then-custom sequence across both filters.
...
- An HTTP 403 response with the `rest_cookie_invalid_nonce` error code for a cookie-authenticated request that supplied an invalid or expired `X-WP-Nonce`. A missing nonce instead makes Core treat the REST request as anonymous; any resulting denial comes from downstream permission logic and may carry a different code/status.
...
- A missing nonce on a cookie-bearing request, causing Core to evaluate it as anonymous and any protected route to deny it downstream; or a supplied invalid/expired nonce, causing the distinct `rest_cookie_invalid_nonce` HTTP 403 result. A long-lived page can encounter the latter when its previously valid nonce expires.
...
5. Where cookie authentication is in use, distinguish absence from invalidity. If no `X-WP-Nonce` (or `_wpnonce`) is supplied, Core sets the REST current user to anonymous; inspect the downstream permission response. If a nonce is supplied but stale, malformed, or otherwise invalid for the active session, Core returns `rest_cookie_invalid_nonce` with HTTP 403.
6. Where Application Passwords are in use, confirm the specific application password has not been revoked or regenerated, that Core considers the mechanism available (SSL or a `local` environment by default, subject to `wp_is_application_passwords_available`), and that the `Authorization` header actually arrives intact at PHP rather than being stripped by an intermediate proxy, CDN, or web-server configuration.
...
- Where no nonce was sent by a legitimate cookie-authenticated client, correct the client to send one generated for the active session. Where a supplied nonce is invalid or expired, obtain a fresh nonce (for example, by reloading the page that generates it) rather than retrying the same value.
...
This entry's governing direction was `SF-TAXONOMY-002` Version 1.2, whose Section 3 declaration and Section 4 argument-validation placement decision are both directly applicable here and are applied without narrowing or widening either: the taxonomy explicitly assigns argument/schema validation to this entry rather than `WP-ERROR-023`, on the basis of observable engineering state (the callback's own business logic has not yet begun) rather than WordPress's internal invocation order, and that decision is reflected throughout this entry's Sections 4, 6, 8, and 9. The specific technical grounding includes the `rest_forbidden`/`rest_cookie_invalid_nonce`/`rest_invalid_param`/`rest_missing_callback_param` error codes, `rest_authorization_required_code()`'s 401-versus-403 distinction, Application Passwords' SSL-or-local default and availability filters, the missing-versus-invalid nonce paths, and the distinct `determine_current_user`/`rest_authentication_errors` callbacks used by Core authentication mechanisms.
...
**Version 1.1 (2026-07-16):** post-certification correction through **SF-SPEC-013** Section 5.6, prompted by the source gate preceding `WP-VERIFICATION-008`. Corrected the missing-versus-invalid nonce distinction, Application Passwords' SSL-or-local default availability, and the inaccurate universal authentication-handler sequence. Request-acceptance ownership, taxonomy scope, recovery safety, and the WP-ERROR-021/023 boundaries are unchanged. Reviewed via `SF-REVIEW-189`/`190`; REST API re-certified via `SF-REVIEW-191`/`192` as Knowledge Baseline v3.
