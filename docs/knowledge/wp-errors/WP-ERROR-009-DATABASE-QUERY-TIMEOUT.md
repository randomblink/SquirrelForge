...
**Covered:** A verified condition in which the database connection was successfully established, authenticated, and privileged, and a specific query subsequently failed to complete within an applicable time or resource limit, whether enforced by the database server's own statement-execution limit, the PHP database driver's own timeout, PHP's own script execution-time limit, or a web-server or gateway-level request timeout.
...
- PHP's own script execution-time limit (`max_execution_time` in `php.ini`, or `set_time_limit()`), which — although a general PHP setting, not specific to the database — commonly manifests as a query timeout when a single slow query consumes most or all of a request's available execution time. This setting shares an identical name with MySQL's own `max_execution_time` system variable, but the two are entirely unrelated.
- Web-server or gateway-level request-timeout settings (for example, PHP-FPM's `request_terminate_timeout`, or a reverse proxy's read timeout), which can terminate the request due to the same underlying slow query before PHP's own execution-time limit would otherwise trigger.
...
- Document the timeout values configured at each layer (database statement timeout, driver timeout, PHP `max_execution_time`, and any gateway or proxy timeout) and their relationship to each other, so a future diagnosis does not need to rediscover which layer is actually enforcing a given cutoff.
