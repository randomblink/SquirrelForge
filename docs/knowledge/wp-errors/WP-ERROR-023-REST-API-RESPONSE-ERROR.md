...
A WordPress REST API request has been fully accepted — its route resolved (`WP-ERROR-021`) and its authentication, authorization, and arguments all validated (`WP-ERROR-022`) — and the route's own callback begins executing its business logic, but the resulting response is not a valid, successful result: the callback returns a `WP_Error`, an uncaught exception or PHP fatal error occurs during its execution, or it returns a value that cannot be turned into a valid REST response.

---

# 4. Primary Failure Mode

The request has cleared every gate `WP-ERROR-021` and `WP-ERROR-022` own, and the route's own callback begins running its actual business logic. This entry covers three distinct ways that execution can fail to produce a valid, successful response:
...
- **The callback returns, without error, a value that cannot be turned into a valid REST response** — for example, a resource, a closure, or data containing values that cannot be represented in JSON. `rest_ensure_response()` normally wraps an ordinary return value (a `WP_REST_Response`, a `WP_HTTP_Response`, or plain data such as an array) into a proper response object, but content that is not actually serializable can still produce a malformed or incomplete response once WordPress attempts to encode it as JSON.
...
- **An invalid/non-serializable return value versus the other two:** this is a distinct, "quiet" failure mode — the callback does not throw, and does not return a `WP_Error`, so nothing about its own execution signals a problem; the failure only becomes visible once WordPress attempts to encode the returned value as JSON and cannot.
...
- **Plugin category**: where a specific plugin's own coding defect is the reason its own callback throws, fatals, or returns an unserializable value, that defect belongs to the Plugin category. This entry owns the observable condition "the REST response was invalid or malformed," not the underlying code defect that produced it.

---

# 7. Scope

**Covered:** A verified condition in which a REST request has been fully accepted (route resolved, authenticated, authorized, and its arguments validated) and the callback's own business logic began executing, but the result is a `WP_Error`, an uncaught exception or PHP fatal error during execution, or a value that cannot be turned into a valid REST response — regardless of the underlying root cause of that failure.
...
- `wp_json_encode()` (and the underlying PHP `json_encode()`), which can fail or produce unexpected output when the data a callback returns contains a value it cannot represent (a resource, a closure, invalid UTF-8, or a circular reference).
...
- A response that appears superficially valid (a 200 status) but whose body is truncated, empty, or fails to parse as JSON on the calling side, indicating a non-serializable return value rather than an outright execution failure.
...
- A callback constructing a response containing invalid UTF-8 or a circular reference, causing JSON encoding to fail or produce unexpected output.
...
3. Determine whether the response is a well-formed JSON error (a `WP_Error`-derived response with its own specific, non-generic error code) or a broken/non-JSON response (HTML, blank, or raw PHP fatal-error output), since the two point toward materially different diagnosis paths: the former is the callback's own deliberate error-reporting, while the latter indicates an uncaught exception or fatal error the callback itself did not anticipate or handle.
...
- The previously failing request now returns a valid, well-formed REST response — either a genuine success, or a clean `WP_Error`-derived error with an accurate, specific code and message — rather than a broken or non-JSON response.
...
- No equivalent uncaught exception, PHP fatal error, or malformed response recurs across repeated, fresh requests to the same and related endpoints.
