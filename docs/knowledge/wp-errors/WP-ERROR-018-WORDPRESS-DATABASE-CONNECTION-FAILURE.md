...
This entry documents the general, verified observable condition of a database connection attempt failing, and the specific, dedicated way WordPress handles that condition (`dead_db()` / `wp_die()`, optionally customized by `wp-content/db-error.php`) rather than as an uncaught PHP fatal error. It does not claim to own the specific, verified causes of a connection failure once they have been isolated; those are reserved for separate, cause-specific entries named in Section 6. WP-ERROR-002, WP-ERROR-003, WP-ERROR-007, and WP-ERROR-008 now exist in this repository; the remaining causes named in Section 6 (WP-ERROR-004, 005, 006, and 009) do not yet exist. Consistent with the single-responsibility principle in **SF-SPEC-001** Section 4.3, this entry owns the general condition and the diagnostic process for narrowing it to a specific cause, not the full remediation detail for every possible specific cause.
