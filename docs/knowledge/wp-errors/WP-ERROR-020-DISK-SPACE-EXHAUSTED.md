...
- **PHP/WordPress upload-size limits**: see the internal distinction above. Excluded entirely from this entry's scope, per `SF-TAXONOMY-001` Section 4.
...
- The `wp-content/upgrade` staging directory, used during plugin and theme installation and updates; its exhaustion (like its inaccessibility under `WP-ERROR-019`) produces WordPress's own "Installation Failed: Could Not Create Directory." message.
...
- WordPress's own "Installation Failed: Could Not Create Directory." message during plugin or theme installation or update — also shared with `WP-ERROR-019`.
