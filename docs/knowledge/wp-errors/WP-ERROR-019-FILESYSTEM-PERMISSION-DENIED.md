...
- **`WP-ERROR-020` — WordPress Disk Space Exhausted**: presumes the operating system is willing to grant the requested access, with the failure occurring because no capacity remains to complete the write. This entry presumes capacity is not the issue; access itself is denied regardless of available space.
...
- A write that fails because of exhausted volume capacity, an applicable quota, or exhausted inodes, where the operating system would otherwise grant the requested access (see [WP-ERROR-020](WP-ERROR-020-DISK-SPACE-EXHAUSTED.md)).
