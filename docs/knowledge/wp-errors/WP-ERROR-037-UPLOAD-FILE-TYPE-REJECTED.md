...
1. **Extension not in the allowed-types list** — the file's own extension does not appear in the currently-applicable, filtered allowed-types list at all. This list is not a single, fixed set: it can be narrowed on a multisite installation relative to a single-site default, and it is itself broader for a user holding the `unfiltered_upload` capability — granted only where the `wp-config.php` constant `ALLOW_UNFILTERED_UPLOADS` is explicitly defined `true` (and, on multisite, only to a super admin even then); absent that constant, no user, including an unmodified Administrator, holds this capability by default — than for one who does not. The *same* file can therefore be accepted for one user and rejected for another only where that constant is actually set; a diagnostic detail worth confirming directly (Section 11) rather than assuming any user holds this capability by default.
...
- The `unfiltered_upload` capability, which, where granted, permits a user to bypass the standard allowed-types restriction. Granting is not automatic for any role: WordPress core's own `map_meta_cap()` (`wp-includes/capabilities.php`) grants this capability only where the `wp-config.php` constant `ALLOW_UNFILTERED_UPLOADS` is explicitly defined `true` (and, on multisite, only to a super admin even then) — absent that constant, no user, including an unmodified Administrator, holds this capability.
- The `ALLOW_UNFILTERED_UPLOADS` `wp-config.php` constant, the sole mechanism that grants the `unfiltered_upload` capability at all — not commonly defined, since doing so is itself a deliberate expansion of the attack surface described in Section 15.
- The `DISALLOW_UNFILTERED_UPLOADS` `wp-config.php` constant, which, where defined `true`, revokes the `unfiltered_upload` capability network- or site-wide regardless of role — relevant only where `ALLOW_UNFILTERED_UPLOADS` was also set, since without it the capability is already absent.
...
- WordPress's own "Sorry, you are not allowed to upload this file type." message, the most common user-facing symptom of this entry's own condition.

---

# 9. Typical Symptoms

- WordPress's own "Sorry, you are not allowed to upload this file type." message, or an equivalent rejection, for a file within any applicable size limit.
...
3. **Determine which of the two causes applies**: first confirm whether `ALLOW_UNFILTERED_UPLOADS` is even defined `true` in the active `wp-config.php` — since without it, no user on the installation holds `unfiltered_upload` regardless of role, and this diagnostic branch does not apply at all. Where it is defined, attempt the identical upload as a user known to hold the capability (on multisite, only a super admin qualifies even with the constant set) to isolate whether the rejection is capability-dependent (cause 1) or occurs regardless of the uploading user (pointing toward either a genuine allowed-types absence, cause 1, or a content mismatch, cause 2). Where the identical upload still fails for such a user, confirm whether `DISALLOW_UNFILTERED_UPLOADS` is also defined `true`, which revokes the capability regardless of role even when `ALLOW_UNFILTERED_UPLOADS` is set.
...
**Version 1.1 (2026-07-15):** corrected via **SF-SPEC-013** Section 5.6's post-certification change process, the first entry-level correction under that process in this catalog. `WP-VERIFICATION-001` — the first Reference Implementation runtime-verification record (`docs/knowledge/verifications/`), executed against a real, disposable local WordPress 7.0.1 installation — found this entry's documented WordPress rejection message ("Sorry, this file type is not permitted for security reasons.") does not exist anywhere in WordPress core; the actual message, confirmed both by direct trigger and by source (`wp-includes/functions.php`), is "Sorry, you are not allowed to upload this file type." The same record found this entry documented only the `unfiltered_upload` capability's revocation (`DISALLOW_UNFILTERED_UPLOADS`) and described the capability itself as merely "uncommon by default," when core source (`wp-includes/capabilities.php`) shows the capability requires an explicit, previously-undocumented `ALLOW_UNFILTERED_UPLOADS` constant to be granted at all — absent by default for every user, including an unmodified Administrator. Both corrections reviewed via `SF-REVIEW-149` (Class A) and `SF-REVIEW-150` (Class B); Media category re-certified via `SF-REVIEW-151` (consistency) and `SF-REVIEW-152` (baseline). The entry's mechanism, boundary, and two-cause partition were independently re-verified as accurate by the same runtime record; only these two implementation-level details required correction.
