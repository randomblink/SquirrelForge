...
2. WP-ERROR-026 — WordPress Capability or Role Authorization Denied (conceptual reference; planned per `SF-TAXONOMY-003` Section 3, no corresponding document currently exists in this repository; no link is provided) — see Section 6 (Distinction) above for how a post-session-validity authorization decision differs from this entry's own condition.
